...
impl Checker<'_> {
    ...
    fn check_in(&mut self, expr: &Expr, lhs: &Expr, rhs: &Expr) -> Option<TypedExpr> {
        ...
        let ok = match haystack.ty() {
            // §optional-membership: an `T?` needle is accepted against a
...
/// `T?` compares only against a present value of its own base type `T`
/// (`int?` vs `int`) or against `json`, so a bare `none`
/// (`json?`) compares against a `json` value or an identical
/// `json?` — never against an arbitrary optional. Comparing a typed
/// `T?` to `none` with `==` is therefore rejected; `has()` is the
