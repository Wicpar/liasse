...
//! shape of a set is never `T?`") say the same for set members: "`none`
//! is **not a member** … never a valid set element."
//!
//! `decode.rs::decode_map` and `decode.rs::decode_set` are the codec's last line
//! of defense: even though the model now REJECTS `map<K, V?>` and
//! `{ $set: T? }` at build (see the sibling runtime reachability probe), an
...
fn decode_set_optional_element_must_not_store_a_none_member() {
    ...
    // and the model accepts `{ $set: T? }`; a `null` element decodes to a
...
fn control_top_level_optional_null_decodes_to_none_correctly() {
    // The LEGITIMATE `none` position: a top-level `text?` decodes a wire
