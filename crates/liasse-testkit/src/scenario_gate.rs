...
pub const SKIP: &[(&str, &str)] = &[
    ...
    ("13-modules/cross-module-atomic-transition", "§13.10 the `#credits` `$use` peer-alias dispatch now RESOLVES and folds atomically on the in-memory coordinator (proven directly by the runtime `multiengine_atomic` peer-alias test); the step-4 call still rejects on this memory gate for a DISTINCT reason — the scenario adapter forwards the child mutation's `@cost` argument untyped (a JSON string decodes as `text`, adapter/wire.rs `infer`), so the bank's decimal `balance >= @amount` metered assert fails; that is an adapter arg-typing seam, not the peer resolution. The durable PostgreSQL multi-instance commit remains the deferred Part-B follow-up behind the loud `commit_folded` `Unsupported` seam; the case flips to `ok` once the adapter types the forwarded arg AND PG parity lands"),
