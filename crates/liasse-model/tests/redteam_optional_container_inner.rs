...
//! Red-team regression: an `T?` set-element / map-value shape is a
//! STATIC error with a rustc-like diagnostic.
//!
//! SPEC.md §5.5 (line 489): "the member shape of a set is never `T?`."
...
fn map_value_optional_string_form_is_rejected() {
    let built = build(&field_model(r#""meta": "{ $key: text, $value: text? }""#));
    let rendered = built.rendered();
    assert!(built.has_code("M-TYPE"), "a map with an optional `$value` is a type error (A.1):\n{rendered}");
    assert!(
        rendered.contains("map `$value`") && rendered.contains("optional"),
        "A.1 line 4400: the diagnostic must explain a map `$value` type is never optional:\n{rendered}"
    );
}

#[test]
fn set_element_optional_string_form_is_rejected() {
    let built = build(&field_model(r#""tags": "{ $set: text? }""#));
    let rendered = built.rendered();
    assert!(built.has_code("M-TYPE"), "a set with an optional element is a type error (§5.5):\n{rendered}");
    assert!(
        rendered.contains("set element") && rendered.contains("optional"),
        "§5.5 line 489: the diagnostic must explain a set element type is never optional:\n{rendered}"
    );
...
fn set_element_optional_inline_form_is_rejected() {
    // The inline `{ $set: "text?" }` element resolves through
    // `scalar_shape` (build/shapes.rs `shape_or_type`), a distinct path from the
    // `set<text?>` string form (which `map_type` catches). It needs its
    // own guard — pinned here.
    let built = build(&field_model(r#""tags": { "$set": "text?" }"#));
    let rendered = built.rendered();
    assert!(built.has_code("M-TYPE"), "an inline `{{ $set: \"T?\" }}` element is a type error (§5.5):\n{rendered}");
    assert!(
        rendered.contains("set element") && rendered.contains("optional"),
        "§5.5 line 489: the inline diagnostic must explain a set element type is never optional:\n{rendered}"
    );
}
...
fn map_and_set_of_a_struct_with_an_optional_member_still_load() {
    ...
    build(&field_model(r#""meta": "{ $key: text, $value: { a: text, b?: text } }""#)).expect_ok();
    // And the plain non-optional collections remain valid.
    build(&field_model(r#""tags": "{ $set: text }""#)).expect_ok();
    build(&field_model(r#""meta": "{ $key: text, $value: text }""#)).expect_ok();
}
