...
//! Red-team regression: an `K?` map KEY shape is a STATIC error with a
...
//! for optionality, silently accepting an optional map `$key`. Enforcing the rule at
...
fn map_key_optional_string_form_is_rejected() {
    let built = build(&field_model(r#""index": "{ $key: text?, $value: text }""#));
    let rendered = built.rendered();
    assert!(built.has_code("M-TYPE"), "a map with an optional `$key` is a type error (A.1):\n{rendered}");
    assert!(
        rendered.contains("map key") && rendered.contains("optional"),
        "A.1: the diagnostic must explain a map `$key` type is never optional:\n{rendered}"
    );
...
fn plain_map_keys_still_load() {
    ...
    build(&field_model(r#""index": "{ $key: text, $value: text }""#)).expect_ok();
    build(&field_model(r#""index": "{ $key: uuid, $value: int }""#)).expect_ok();
}
