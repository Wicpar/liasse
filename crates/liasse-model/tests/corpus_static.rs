...
const SKIP: &[(&str, &str)] = &[
    ...
    (
        ...
        "the §16.5 app-namespace-in-`$verify` rule is enforced at load in the RUNTIME (`Compiled::build` audits every authenticator `$verify`/`$actor`/`$session`/`$check` position, `liasse-runtime` `redteam_mandate7_auth_position`), not the CORE static model: the CORE model only parse-checks an authenticator `$verify` (auth.rs `parse_only`), and this static bridge builds the model with NO host descriptors (`Model::build`), so `test.auth` is unresolved and the native/deferred verifier substitution (§11 corpus, w2) is correctly tolerated — the resolved-verifier rejection has no host context to fire against here — like the other skipped §11/§16 auth/host seams",
    ),
