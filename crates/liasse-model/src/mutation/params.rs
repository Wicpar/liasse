...
use super::helpers::{
    arg_expr, is_scalar_binop, local_binding_name, record, stmt_exprs, BindEnv, Params,
};
use super::host_args::HostArgInference;
...
impl<'a> Inference<'a> {
    ...
    pub(super) fn infer(
        ...
        // host-namespace call argument — the login shape
        // `identity = webauthn.verify(@response)` uses `@response` nowhere else, so
        // it must become a real contract parameter that the caller passes explicitly
        // in the §12.1 closed argument object, and a parameter nested in a structured
        // argument (`ns.fn({ field: @p })`) takes the descriptor type that position
        // declares ([`super::host_args`]). Running
        ...
        if let Some(hosts) = self.hosts {
            let host_args = HostArgInference::new(hosts);
            for stmt in statements {
                for expr in stmt_exprs(stmt) {
