...
**Module updates.** A single-instance module update (§13.14) MAY be exposed the same way, and is held to the same rule: prepare and apply MUST NOT be separate computations of it, so its dry run reports what it does. Preparing one performs the §13.14 exposed-surface recheck and the §13.15 `$exposed` grouping in addition to the whole of §20.1 over that instance, and the prepared module update carries the instance's own prepared update together with those §13.15 facts. Its basis additionally names the **mount**, the module-collection entry (§13.2) the instance was prepared at, because a module update addresses an instance through that entry: an apply MUST refuse a prepared module update whose mount now holds a different incarnation (D.1) or holds none. The surrounding composition is not part of that basis — a single-instance update reads the instance's active definition, the target, and the instance's own state, so a sibling installed or a peer rebound (§13.5) in between changes nothing the prepared update computed, and a prepared module update asserts nothing about it. Every limit above carries across unchanged: a newly declared keyring is still provisioned by the apply, and the reported §13.13 conflicts are still advisory rather than blocking.
...
Whether an operation whose committed response a client has observed precedes any operation that client subsequently issues on another connection is an implementation-declared property of the admission mechanism; a conforming implementation MUST document whether it provides it. This reference implementation does. Admission itself assigns no position; it records the transition and commits. Serial positions are assigned afterwards, over transitions that have **settled**, in the order they entered admission, and a position is never assigned while an earlier admission could still claim a place before it. So a position exists only for a transition that has definitely committed, and an operation issued after an observed commit entered admission after it and is ordered after it — a client that appends on one connection, waits for that commit, then appends on a second connection observes the first append ordered before the second.
