...
- Every `dyn` trait object we define carries a `Send + Sync` supertrait bound so `Engine<S>` stays `Send + Sync` for a `Send + Sync` store; a new trait used as `dyn` must add it. Pinned by the assertions in `crates/liasse-runtime/src/send_sync.rs` (the `MemoryStore` engine and the boxed `HostNamespace`/`KeyProvider`/`BlobConnector`/`ViewProgram`) and `crates/liasse/src/send_sync.rs` (the pg engine). The sole exception is a `dyn` trait whose implementor transitively holds the store `S` — `StateReader` (over `EngineReader<'_, S>`) and the testkit-internal `Instance` (over `Runtime<S>`): bounding these would force `S: Sync`, but `PgStore`'s single-writer `postgres::Client` is intentionally `Send`-not-`Sync` (§5.2), and `Engine`'s own `Send + Sync` does not depend on them, so they stay bare.
