...
| `module_update` | `{ module_update: { instance: "<path>", to: "name@version", dry_run?: true }, expect }` — §13.14/§13.15 single-instance update; `to` resolves against the `packages` map like `$module`. On success `expect.value` matches the §13.15 update report. The optional `dry_run: true` member selects the §20.4 **dry run** of that same update: it is computed in full and then discarded, so the step reports the outcome the update would have — same vocabulary, same mapping — and applies nothing (no migration, no commit, no version movement). A dry run takes no commit, so it carries no §13.15 report value; later steps assert against the release that was already mounted. |
